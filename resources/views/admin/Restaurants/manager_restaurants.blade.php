...
@section('admin_content')
        ...
        <div class="card">
            <div class="card-body">
                <div style="display: flex;justify-content: space-between">
                    <div class="card-title col-sm-6">Bảng Danh Sách Nhà Hàng</div>
                    <div class="col-sm-3">
                        <div class="input-group">
                            <input id="search" type="text" class="form-control" name="search"
                                placeholder="Tìm Kiếm ID Hoặc Tên Nhà Hàng">
                    ...
                    <div class="col-sm-2">
                        ...
                        <div class="input-group">
                            <a style="text-decoration: none" href="{{ URL::to('admin/restaurants/list-deleted-restaurant') }}">
    ...
    <script>
        ...
        function getPosts(page) {
            $.ajax({
                url: '{{ url('/admin/restaurants/load-restaurants?page=') }}' + page,
                method: 'get',
                data: {

                },
                success: function(data) {
                    $('#load_hotel').html(data);
                },
                error: function() {
                    alert("Bug Huhu :<<");
                }
    ...
    <script>
        $('#search').keyup(function() {
            ...
            $.ajax({
                url: '{{ url('/admin/hotel/search-hotel') }}',
                method: 'GET',
                data: {
        ...
        $(document).on('click', '.update-status', function() {
            ...
            $.ajax({
                ...
                url: '{{ url('/admin/hotel/update-status-hotel') }}',
                method: 'POST',
                data: {
                    hotel_id: item_id,
                    hotel_status: item_status,
                },
                success: function(data) {
                    getPosts(notePage);
                    if (item_status == 1) {
                        message_toastr("success", 'Slider ID ' + item_id + ' Đã Được Kích Hoạt!');
                    } else if (item_status == 0) {
                        message_toastr("success", 'Slider ID ' + item_id + ' Đã Bị Vô Hiệu!');
                    }
                },
                error: function() {
                    alert("Bug Huhu :<<");
        ...
        $(document).on('click', '.btn-delete-item', function() {
            var item_id = $(this).data('item_id');
            message_toastr("success", 'Xác Nhận Xóa Slider ID ' + item_id +
        ...
        function move_to_bin(item_id) {
            $.ajax({
                ...
                url: '{{ url('/admin/hotel/delete-hotel') }}',
                method: 'POST',
                data: {
                    hotel_id: item_id,
                },
                success: function(data) {
                    ...
                    load_count_bin();
                    getPosts(notePage);
                    message_toastr("success", 'Slider ID ' + item_id + ' Đã Được Đưa Vào Thùng Rác !');
                },
                error: function() {
                    $(".loading").css({
                        "display": "none"
