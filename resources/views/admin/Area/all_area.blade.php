...
@section('admin_content')
        ...
        <div class="card">
            <div class="card-body">
                <div style="display: flex;justify-content: space-between">
                    <div class="card-title col-sm-6">Bảng Danh Sách Khu Vực</div>
                    <div class="col-sm-3">
                        <div class="input-group">
                            <input id="search" type="text" class="form-control" name="search"
                                placeholder="Tìm Kiếm ID Hoặc Tên Khu Vực">
                    ...
                    <div class="col-sm-2">
                        ...
                        <div class="input-group">
                            <a style="text-decoration: none" href="{{ URL::to('admin/area/list-deleted-area') }}">
    ...
    <script>
        ...
        function getPosts(page) {
            $.ajax({
                url: '{{ url('/admin/area/load-area?page=') }}' + page,
                method: 'get',
                data: {
    ...
    <script>
        $('#search').keyup(function() {
            ...
            $.ajax({
                url: '{{ url('/admin/area/search-area') }}',
                method: 'GET',
                data: {
        ...
        $(document).on('click', '.update-status', function() {
            ...
            $.ajax({
                ...
                url: '{{ url('/admin/area/update-status-area') }}',
                method: 'POST',
                data: {
                    area_id: item_id,
                    area_status: item_status,
                },
                success: function(data) {
                    getPosts(notePage);
                    if (item_status == 1) {
                        message_toastr("success", 'Khu Vực ID ' + item_id + ' Đã Được Kích Hoạt!');
                    } else if (item_status == 0) {
                        message_toastr("success", 'Khu Vực ID ' + item_id + ' Đã Bị Vô Hiệu!');
                    }
                },
                error: function() {
                    alert("Bug Huhu :<<");
        ...
        $(document).on('click', '.btn-delete-item', function() {
            var item_id = $(this).data('item_id');
            message_toastr("success", 'Xác Nhận Xóa Khu Vực ID ' + item_id +
        ...
        function move_to_bin(item_id) {
            $.ajax({
                ...
                url: '{{ url('/admin/area/delete-area') }}',
                method: 'POST',
                data: {
                    area_id: item_id,
                },
                success: function(data) {
                    ...
                    load_count_bin();
                    getPosts(notePage);
                    message_toastr("success", 'Khu Vực ID ' + item_id + ' Đã Được Đưa Vào Thùng Rác !');
                },
                error: function() {
                    $(".loading").css({
                        "display": "none"
