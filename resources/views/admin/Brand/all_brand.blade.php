...
@section('admin_content')
        ...
        <div class="card">
            <div class="card-body">
                <div style="display: flex;justify-content: space-between">
                    <div class="card-title col-sm-6">Bảng Danh Sách Thương Hiệu</div>
                    <div class="col-sm-3">
                        <div class="input-group">
                            <input id="search" type="text" class="form-control" name="search"
                                placeholder="Tìm Kiếm ID Hoặc Tên Thương Hiệu">
                    ...
                    <div class="col-sm-2">
                        ...
                        <div class="input-group">
                            <a style="text-decoration: none" href="{{ URL::to('admin/brand/list-deleted-brand') }}">
    ...
    <script>
        ...
        function getPosts(page) {
            $.ajax({
                url: '{{ url('/admin/brand/load-brand?page=') }}' + page,
                method: 'get',
                data: {

                },
                success: function(data) {
                    $('#load_brand').html(data);
                },
                error: function() {
                    alert("Bug Huhu :<<");
                }
    ...
    <script>
        $('#search').keyup(function() {
            ...
            $.ajax({
                url: '{{ url('/admin/brand/search-brand') }}',
                method: 'GET',
                data: {
        ...
        $(document).on('click', '.update-status', function() {
            ...
            $.ajax({
                ...
                url: '{{ url('/admin/brand/update-status-brand') }}',
                method: 'POST',
                data: {
                   brand_id: item_id,
                   brand_status: item_status,
                },
                success: function(data) {
                    getPosts(notePage);
                    if (item_status == 1) {
                        message_toastr("success", 'Thương Hiệu ID ' + item_id + ' Đã Được Kích Hoạt!');
                    } else if (item_status == 0) {
                        message_toastr("success", 'Thương Hiệu ID ' + item_id + ' Đã Bị Vô Hiệu!');
                    }
                },
                error: function() {
                    alert("Bug Huhu :<<");
        ...
        $(document).on('click', '.btn-delete-item', function() {
            var item_id = $(this).data('item_id');
            message_toastr("success", 'Xác Nhận Xóa Thương Hiệu ID ' + item_id +
        ...
        function move_to_bin(item_id) {
            $.ajax({
                ...
                success: function(data) {
                    ...
                    load_count_bin();
                    getPosts(notePage);
                    message_toastr("success", 'Thương Hiệu ID ' + item_id + ' Đã Được Đưa Vào Thùng Rác !');
                },
                error: function() {
                    $(".loading").css({
                        "display": "none"
